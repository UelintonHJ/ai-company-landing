        ...
        <>
            <header className={`fixed left-1/2 top-0 z-50 mt-3 flex w-[min(95%,72rem)] -translate-x-1/2 items-center justify-between rounded-2xl border px-4 py-3 backdrop-blur-xl transition-all duration-300 ${isScrolled ? 'border-blue-400/30 bg-black/85 shadow-xl shadow-blue-500/10' : 'border-zinc-800/80 bg-zinc-900/70 hover:border-zinc-700'}`}>
                ...
                <nav className="hidden items-center gap-6 text-sm text-zinc-300 md:flex">
                    <a href="#features" className="transition hover:text-blue-300">Recursos</a>
                    <a href="#pricing" className="transition hover:text-blue-300">Preços</a>
                    <a href="#about" className="transition hover:text-blue-300">Sobre</a>
            ...
            {menuOpen && (
                <div className="fixed left-1/2 top-20 z-40 w-[min(95%,72rem)] -translate-x-1/2 rounded-2xl border border-zinc-800 bg-zinc-900/95 p-4 md:hidden">
                    <div className="flex flex-col gap-3 text-sm text-zinc-300">
                        <a href="#features" onClick={onCloseMenu} className="transition hover:text-blue-300">Recursos</a>
                        <a href="#pricing" onClick={onCloseMenu} className="transition hover:text-blue-300">Preços</a>
                        <a href="#about" onClick={onCloseMenu} className="transition hover:text-blue-300">Sobre</a>
                        <a href="#faq" onClick={onCloseMenu} className="transition hover:text-blue-300">FAQ</a>
                        <button onClick={onCloseMenu} className="mt-2 inline-flex items-center justify-center gap-2 rounded-lg bg-blue-500 px-4 py-2 font-semibold text-white">
                            <FaEnvelope />
                            Agende uma demonstração
                        </button>
                    </div>
